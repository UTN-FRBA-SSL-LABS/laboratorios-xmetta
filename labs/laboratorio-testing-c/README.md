...
> R: No coincide el total que imprime el programa, imprime lo siguiente:
...
> R: Inicializa el carrito en 0 y es importante porque si no sería un vector basura.
...
> R:El test falla, esperaba 700 y obtuvo 350.
...
> R: El bug está en la línea 24, esa línea suma solo el precio de cada producto, ignorando por completo cuántas unidades hay.

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Debería multiplicar precio por cantidad
...
> R: El test falla,devuelve 1 (éxito) en vez de 0, y encima el programa termina en Segfault por escribir fuera del array.
...
> R: el operador incorrecto es "<=", debería ser "<".
...
> R:Sí, hay líneas con #####: son las líneas 29 y 30, que forman toda la función carrito_descuento, no se ejecutaron porque "make cobertura" solo corre "test_unitarios.c", y ninguno de esos tests llama a "carrito_descuento".
...
> R: Un test unitario prueba una función aislada, con pocas dependencias (por ejemplo, test_total_con_cantidad solo usa carrito_init, carrito_agregar y carrito_total). Un test de integración prueba varias funciones trabajando juntas, simulando un caso de uso más real (como test_compra_con_descuento, que combina carrito_agregar, carrito_total y carrito_descuento). El test unitario fue el que detectó primero el bug de carrito_total.
...
> R: En C, el compilador y el runtime no verifican que un índice esté dentro de los límites del array. Si escribís "items[4]" en un array de tamaño 4, C te deja hacerlo sin quejarse, simplemente escribe en la memoria que está justo después del array, pisando datos que no le pertenecen (por eso el segmentation fault). En Python o Java, en cambio, el runtime sí chequea los índices en cada acceso, y si te pasás del límite tira un error controlado (IndexError en Python, ArrayIndexOutOfBoundsException en Java) en vez de corromper memoria silenciosamente. Esa falta de chequeo automático es lo que hace que los buffer overflows en C sean peligrosos, pueden causar crashes impredecibles.
...
> R: Mirando el código de carrito_total a simple vista, era fácil no notar que faltaba el * cantidad, la línea total compila perfecto y no tiene nada "raro" a primera vista. El test, en cambio, detectó que lo que faltaba con el esperado 700, obtenido 350. Escribir tests obliga a pensar y definir qué resultado exacto se espera, y eso hace mucho más difícil que un bug pase desapercibido, comparado con solo leer el código y confiar en que "parece que está bien".
...
> R: Con el bug (total += precio, sin multiplicar por cantidad), el resultado es siempre igual a la suma de los precios, sin importar la cantidad. Con cantidad = 1, precio * 1 da exactamente lo mismo que precio solo (350 = 350), así que el test pasó "de casualidad", el bug no se notaba porque cantidad=1 es un caso donde multiplicar o no multiplicar da igual. Con cantidad = 2, en cambio, precio * 2 (700) es distinto de precio solo (350), ahí es donde el bug quedó se muestra. Esto muestra que elegir cantidad=1 como único caso de prueba es un mal caso de test, no cubriendo el comportamiento real de la multiplicación.
